...
class OperationQuery < CrmQuery
  ...
  def initialize_available_filters
    ...
    add_available_filter("category_id", :type => :list, :label => :label_operation_category,
      :values => OperationCategory.order("#{OperationCategory.table_name}.is_income DESC").order(:position).map{|a| [a.name, a.id.to_s]}
    )
  ...
  def sql_for_operation_type_field(field, operator, value)
    ...
    "#{OperationCategory.table_name}.is_income #{op} (#{va})"
  end
